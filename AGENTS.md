...
5. Publishes both packages with `npm publish --provenance` (`bun publish` has no provenance flag). The prototype package's `prepublishOnly` copies `plugin/skills/prototype/` into it as `skill/`, so the published package carries the repinned skill.
